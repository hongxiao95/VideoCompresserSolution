...
namespace VideoCompresserDFW
{
    class VibeAve
    {
        ...
        public void init(Image<Bgr, Byte> _image)
        {
            ...
            m_foreMatchCount = new Byte[_image.Width, _image.Height];
        }
            ...
            for(int i =0; i < _image.Rows; i++)
            {
                for(int j = 0; j < _image.Cols; j++)
                {
                    for(int k =0; k < NUM_SAMPLES; k++)
                    {
                        ...
                        m_samples[k][i, j] = _image[row, col];
                    }
                }
            }
        }                   
        ...
        public void testAndUpdate(Image<Bgr, Byte> _image_bgr)
        {
            ...
            for(int i = 0; i < _image.Rows; i++)
            {
                for(int j =0; j < _image.Cols; j++)
                {
                    ...
                    {
                        dist = Math.Abs(m_samples[count][i, j].Intensity - _image[i, j].Intensity);
                        if(dist < RADIUS)
                        {
                            matchs++;
                        }
                    ...
                    if(matchs >= MIN_MATCHES)
                    {
                        ...
                        if(rdNum == 0)
                        {
                            ...
                            m_samples[rdNum][i, j] = _image[i, j];
                        }
                        ...
                        if(rdNum == 0)
                        {
                            ...
                            rdNum = rd.Next(0, NUM_SAMPLES);
                            m_samples[rdNum][row, col] = _image[i, j];
        ...
        private Image<Gray, Byte> m_mask;
    }
}
